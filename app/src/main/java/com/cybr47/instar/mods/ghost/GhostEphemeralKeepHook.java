        ...
        try {
            ...
            for (MethodData md : methods) {
                try {
                    ...
                    ModuleLog.line("(Instar | Ephemeral) ✅ vanish-local-delete hook → "
                            + md.getClassName() + "." + md.getName());
                    FeatureStatusTracker.setHooked("KeepEphemeralMessages");
                    return;
                } catch (Throwable ignored) {}
            }
            ModuleLog.line("(Instar | Ephemeral) ❌ vanish-local-delete method not found");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Ephemeral) ❌ hookVanishLocalDelete: " + t.getMessage());
        }
    }
    ...
    private void hookServerPing(DexKitBridge bridge) {
        ...
        try {
            ...
            for (MethodData md : methods) {
                ...
                ModuleLog.line("(Instar | Ephemeral) ✅ server-ping hook → "
                        + md.getClassName() + "." + md.getName());
                return;
            }
            ModuleLog.line("(Instar | Ephemeral) ❌ server-ping method not found");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Ephemeral) ❌ hookServerPing: " + t.getMessage());
        }
    }
        ...
        try {
            ...
            for (MethodData md : methods) {
                try {
                    ...
                    XposedBridge.hookMethod(m, hook);
                    hooked.add(m);
                    ModuleLog.line("(Instar | Ephemeral) ✅ expiry-parser hook → "
                            + md.getClassName() + "." + md.getName());
                } catch (Throwable ignored) {}
            }
            if (hooked.isEmpty()) {
                ModuleLog.line("(Instar | Ephemeral) ❌ no expiry-parser methods hooked");
            } else {
                DexKitCache.saveMethods("Ephemeral_expiry", hooked);
            }
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Ephemeral) ❌ hookExpiryParser: " + t.getMessage());
        }
    }
