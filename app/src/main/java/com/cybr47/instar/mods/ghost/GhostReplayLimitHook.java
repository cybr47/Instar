        ...
        try {
            ...
            for (MethodData md : methods) {
                try {
                    ...
                    ModuleLog.line("(Instar | Replays) ✅ update hook → " + md.getClassName() + "." + md.getName());
                    return;
                } catch (Throwable ignored) {}
            }
            ModuleLog.line("(Instar | Replays) ❌ update method not found");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Replays) ❌ hookUpdateMethod: " + t);
        }
    }
        ...
        try {
            ...
            for (MethodData md : methods) {
                try {
                    ...
                    XposedBridge.hookMethod(m, hook);
                    hooked.add(m);
                    ModuleLog.line("(Instar | Replays) ✅ parseFromJson hook → " + md.getClassName() + "." + md.getName());
                } catch (Throwable ignored) {}
            }
            if (hooked.isEmpty()) {
                ModuleLog.line("(Instar | Replays) ❌ parseFromJson method not found");
            } else {
                DexKitCache.saveMethods("Replays_parse", hooked);
                ModuleLog.line("[IE] ✅ Ghost Replay – parseFromJson");
            }
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Replays) ❌ hookParseFromJsonMethod: " + t);
        }
    }
        ...
        try {
            ...
            for (MethodData md : methods) {
                try {
                    ...
                    XposedBridge.hookMethod(m, hook);
                    ModuleLog.line("[IE] ✅ Ghost Replay – sync");
                    ModuleLog.line("(Instar | Replays) ✅ sync hook → " + md.getClassName() + "." + md.getName());
                    FeatureStatusTracker.setHooked("UnlimitedReplays");
                    return;
                } catch (Throwable ignored) {}
            }
            ModuleLog.line("(Instar | Replays) ❌ sync method not found");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Replays) ❌ hookSyncMethod: " + t);
        }
    }
