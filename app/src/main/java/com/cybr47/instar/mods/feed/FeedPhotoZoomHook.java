                    ...
                    try {
                        ...
                        view.setOnLongClickListener(v -> {
                            ...
                            try {
                                ...
                                ModuleLog.line("(Instar | PhotoZoom) ❌ long-press: " + t);
                                return false;
                            }
                        });
                    } catch (Throwable t) {
                        ModuleLog.line("(Instar | PhotoZoom) ❌ attach hook: " + t);
                    }
                }
            });

            FeatureStatusTracker.setHooked("PhotoZoom");
            ModuleLog.line("(Instar | PhotoZoom) ✅ hook installed");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | PhotoZoom) ❌ install: " + t);
        }
    }
    ...
    private static void showZoomOverlay(Context ctx, Bitmap bitmap) {
        MAIN.post(() -> {
            try {
                ...
                ModuleLog.line("(Instar | PhotoZoom) ❌ overlay: " + t);
            }
        });
    }
