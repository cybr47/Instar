...
public class StoryMentionHook {
    ...
    private static void resolveMentionPipeline(DexKitBridge bridge, ClassLoader classLoader) {
        if (DexKitCache.isCacheValid()) {
            ...
            if (g != null && c != null) {
                rawMentionsGetter = g;
                mentionsConverter = c;
                ModuleLog.line("(Instar | Mention) ✅ pipeline resolved from cache");
                return;
            }
        }
        ...
        try {
            ...
            if (rawMentionsGetter == null) ModuleLog.line("(Instar | Mention) ❌ rawMentionsGetter not found");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Mention) ❌ rawMentionsGetter query failed: " + t);
        }
        ...
        try {
            ...
            if (mentionsConverter == null) ModuleLog.line("(Instar | Mention) ❌ mentionsConverter not found");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Mention) ❌ mentionsConverter query failed: " + t);
        }

        if (rawMentionsGetter != null && mentionsConverter != null) {
            ModuleLog.line("(Instar | Mention) ✅ pipeline resolved: " + rawMentionsGetter.getName() + " -> " + mentionsConverter.getName());
        }
    }
    ...
    private void installButtonHook(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        if (method == null) {
            try {
                ...
                ModuleLog.line("(Instar | Mention) ❌ button hook DexKit: " + t);
            }
        }

        if (method == null) {
            ModuleLog.line("(Instar | Mention) ❌ button builder not found");
            return;
        }
        ...
        try {
            ...
            ModuleLog.line("(Instar | Mention) ✅ button hook installed");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Mention) ❌ button hook: " + t);
        }
    }
    ...
    private void installClickHook(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        if (method == null) {
            try {
                ...
                if (methods.isEmpty()) {
                    ModuleLog.line("(Instar | Mention) ❌ click handler not found");
                    return;
                }
                method = methods.get(0).getMethodInstance(classLoader);
                DexKitCache.saveMethod("MentionClick", method);
            } catch (Throwable t) {
                ModuleLog.line("(Instar | Mention) ❌ click hook DexKit: " + t);
                return;
            }
        }
            ...
            XposedBridge.hookMethod(method, new XC_MethodHook() {
                ...
                protected void beforeHookedMethod(MethodHookParam param) {
                    try {
                        ...
                        if (ctx == null) { ModuleLog.line("(Instar | Mention) ❌ context not found"); return; }
                        if (media == null) { ModuleLog.line("(Instar | Mention) ❌ Media not found"); return; }

                        showMentionsDialog(ctx, resolveMentions(media));
                    } catch (Throwable t) {
                        ModuleLog.line("(Instar | Mention) ❌ click handler: " + t);
                    }
                }
            });
            ModuleLog.line("(Instar | Mention) ✅ click hook installed");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Mention) ❌ click hook: " + t);
        }
    }
    ...
    private static List<String> resolveMentions(Object media) {
        ...
        try {
            if (rawMentionsGetter == null || mentionsConverter == null) {
                ModuleLog.line("(Instar | Mention) ❌ mention pipeline not resolved");
                return usernames;
            }

            Object dict = findFieldByType(media, "com.instagram.feed.media.LiveTreeMediaDict");
            if (dict == null) {
                ModuleLog.line("(Instar | Mention) ❌ LiveTreeMediaDict not found on media");
                return usernames;
            }
            ...
            ModuleLog.line("(Instar | Mention) resolveMentions exception: " + t);
        }
        return usernames;
    }
    ...
    private static void showMentionsDialog(Context ctx, List<String> usernames) {
        mainHandler.post(() -> {
            try {
                ...
                ModuleLog.line("(Instar | Mention) ❌ showMentionsDialog: " + t);
            }
        });
    }
