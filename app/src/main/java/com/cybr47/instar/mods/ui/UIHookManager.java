package com.cybr47.instar.mods.ui;

import static org.luckypray.dexkit.query.FindMethod.create;
import static com.cybr47.instar.mods.ghost.ui.GhostEmojiManager.addGhostEmojiNextToInbox;
...
import com.cybr47.instar.utils.dialog.DialogUtils;
...
import com.cybr47.instar.utils.toast.CustomToast;
...
public class UIHookManager {
    ...
    public static void setupHooks(Activity activity) {
        ...
        // a config change (e.g. when Instar settings are toggled), so we cannot skip
    ...
    public void mainActivity(ClassLoader classLoader) {
        ...
        try {
            ...
            if (methods.isEmpty()) {
                ModuleLog.line("(Instar): ⚠️ Specific onCreate not found, searching by signature...");
            ...
            if (!methods.isEmpty()) {
                ...
                if (methodName == null || methodName.isEmpty()) {
                    ModuleLog.line("(Instar): ❌ Invalid onCreate method name discovered");
                } else {
                            ...
                            try {
                                ...
                                new Handler(Looper.getMainLooper()).postDelayed(() -> {
                                    try {
                                        ...
                                        ModuleLog.line("(Instar): UI Injection Error: " + innerE.getMessage());
                                    }
                                }, 1500); // 1.5s delay to let the UI settle

                            } catch (Exception e) {
                                ModuleLog.line("(Instar): UI logic error in onCreate: " + e);
                            }
                        });
                    }
                    });
                ...
                ModuleLog.line("(Instar): ❌ Failed to find any onCreate candidate in InstagramMainActivity");
            }
        } catch (Exception e) {
            ModuleLog.line("(Instar): ❌ DexKit discovery failed: " + e.getMessage());
        }
        ...
        try {
                    ...
                    protected void afterHookedMethod(MethodHookParam param) {
                        ...
                        activity.runOnUiThread(() -> {
                            try {
                                ...
                                ModuleLog.line("(Instar) UI Error: " + e);
                            }
                        });
                    }
                });
            ...
            ModuleLog.line("(Instar): ❌ onResume discovery failed: " + t.getMessage());
        }
    ...
    public static void registerSettingsRestoreReceiver(android.content.Context context) {
        BroadcastReceiver receiver = new BroadcastReceiver() {
            ...
            public void onReceive(android.content.Context ctx, Intent intent) {
                ...
                new Thread(() -> {
                    try {
                        com.cybr47.instar.utils.backup.SettingsBackupManager.fromJson(json);
                        SettingsManager.saveAllFlags();
                        com.cybr47.instar.utils.feature.FeatureManager.refreshFeatureStatus();
        ...
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
                context.registerReceiver(receiver,
                        new IntentFilter("com.cybr47.instar.ACTION_RESTORE_SETTINGS"),
                        android.content.Context.RECEIVER_EXPORTED);
            } else {
                androidx.core.content.ContextCompat.registerReceiver(context,
                        receiver,
