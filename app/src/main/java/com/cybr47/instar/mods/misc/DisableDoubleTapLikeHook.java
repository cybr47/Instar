...
public class DisableDoubleTapLikeHook {
    ...
    public void install(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        if (DexKitCache.isCacheValid()) {
            ...
            if (cachedFeedHooked && cachedGestureReelsHooked) {
                ModuleLog.line("(Instar | DoubleTapLike): Hooked (cached)");
        ...
        try {
            ...
            ModuleLog.line("(Instar | DoubleTapLike): " + e.getMessage());
        }
    }
    ...
    private void findAndHook(DexKitBridge bridge, ClassLoader classLoader, boolean feedAlreadyHooked, boolean reelsAlreadyHooked) {
            ...
            for (MethodData md : feedMethods) {
                try {
                    ...
                    if (hookMethod(method, HOOK)) {
                        feedHooked = true;
                        ModuleLog.line("(Instar | DoubleTapLike): Feed hooked on " + md.getClassName() + "." + md.getMethodName());
                        break;
                    }
                } catch (Exception e) {
                    ModuleLog.line("(Instar | DoubleTapLike): Feed: " + e.getMessage());
                }
            }
            if (feedMethods.isEmpty()) {
                ModuleLog.line("(Instar | DoubleTapLike): Feed method not found");
            }
        }
        ...
        if (hookMethods(reelsGestureMethods, REELS_GESTURE_HOOK) > 0) {
            ...
            reelsHooked = true;
            ModuleLog.line("(Instar | DoubleTapLike): Reels gesture callbacks hooked: " + reelsGestureMethods.size());
        }
            ...
            for (MethodData md : ecgMethods) {
                try {
                    ...
                    if (hookMethod(method, HOOK)) {
                        ...
                        reelsHooked = true;
                        ModuleLog.line("(Instar | DoubleTapLike): Reels legacy hooked on " + cd.getName() + "." + md.getMethodName());
                    }
                } catch (Exception e) {
                    ModuleLog.line("(Instar | DoubleTapLike): Reels: " + e.getMessage());
                }
            }
        }
        if (!reelsHooked) {
            ModuleLog.line("(Instar | DoubleTapLike): Reels entry not found");
        }
        ...
        try {
            ...
            for (MethodData md : callbacks) {
                try {
                    ...
                    ModuleLog.line("(Instar | DoubleTapLike): Reels gesture candidate skipped: " + e.getMessage());
                }
            }
        } catch (Throwable e) {
            ModuleLog.line("(Instar | DoubleTapLike): Reels gesture search failed: " + e.getMessage());
        }
        return methods;
    }
    ...
    private boolean hookMethod(Method method, XC_MethodHook hook) {
        ...
        try {
            ...
            ModuleLog.line("(Instar | DoubleTapLike): Hook failed: " + e.getMessage());
            return false;
        }
    }
