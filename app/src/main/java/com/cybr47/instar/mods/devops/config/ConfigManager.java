    ...
    public static void restoreDefaultConfig(Context context, String moduleApkPath) {
        new Thread(() -> {
            try {
                ...
                ModuleLog.line("Instar | ✅ Default config restored.");
            } catch (Exception e) {
                ModuleLog.line("Instar | ❌ Default config restore failed: " + e.getMessage());
                new Handler(Looper.getMainLooper()).post(() ->
                        Toast.makeText(context.getApplicationContext(),
        ...
        new Thread(() -> {
            try {
                ...
                new Handler(Looper.getMainLooper()).post(() -> {
                    ...
                    ModuleLog.line("Instar | ✅ JSON imported into mc_overrides.json");
                });
            } catch (Exception e) {
                ModuleLog.line("Instar | ❌ Import failed: " + e.getMessage());
                new Handler(Looper.getMainLooper()).post(() ->
