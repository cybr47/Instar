    ...
    public void handleTypingBlock(DexKitBridge bridge) {
        ...
        if (DexKitCache.isCacheValid()) {
            ...
            if (cached != null) {
                ...
                ModuleLog.line("(Instar | TypingBlock): ✅ Hooked (dynamic check): " + cached.getDeclaringClass().getName() + "." + cached.getName());
        ...
        try {
            ...
            if (methods.isEmpty()) {
                ModuleLog.line("(Instar | TypingBlock): ❌ No methods found containing 'is_typing_indicator_enabled'");
                return;
            }
            ...
            for (MethodData method : methods) {
                ...
                if (matchesOldShape || matchesNewShape) {
                    try {
                        ...
                        ModuleLog.line("(Instar | TypingBlock): ✅ Hooked (dynamic check): " +
                                method.getClassName() + "." + method.getName());
                        FeatureStatusTracker.setHooked("GhostTyping");
                        return;

                    } catch (Throwable e) {
                        ModuleLog.line("(Instar | TypingBlock): ❌ Hook error: " + e.getMessage());
                    }
                }
            }

            ModuleLog.line("(Instar | TypingBlock): ❌ No candidate matched the expected method shape");

        } catch (Throwable t) {
            ModuleLog.line("(Instar | TypingBlock): ❌ Exception: " + t.getMessage());
        }
    }
}
