...
public class DisableStoryFlippingHook {
    ...
    public void handleStoryFlippingDisable(DexKitBridge bridge) {
        if (DexKitCache.isCacheValid()) {
            ...
            if (cached != null) {
                ...
                ModuleLog.line("(Instar | StoryFlipping): ✅ Hooked (dynamic check): " + cached.getDeclaringClass().getName() + "." + cached.getName());
                return;
            }
        }
        try {
            ...
            ModuleLog.line("(Instar | StoryFlipping): ❌ Error handling Story Flipping hook: " + e.getMessage());
        }
    }
    ...
    private void findAndHookMethod(DexKitBridge bridge) {
        try {
            ...
            if (methods.isEmpty()) {
                ModuleLog.line("(Instar | StoryFlipping): ❌ No methods found referencing 'end_scene'.");
                return;
            }
            ...
            for (MethodData method : methods) {
                try {
                    ...
                    ModuleLog.line("(Instar | StoryFlipping): ✅ Hooked (dynamic check): " +
                            method.getClassName() + "." + method.getName());
                    return;

                } catch (Exception e) {
                    ModuleLog.line("(Instar | StoryFlipping): ❌ Error hooking method: " + e.getMessage());
                }
            }
        } catch (Exception e) {
            ModuleLog.line("(Instar | StoryFlipping): ❌ Error during dynamic method discovery: " + e.getMessage());
        }
    }
}
