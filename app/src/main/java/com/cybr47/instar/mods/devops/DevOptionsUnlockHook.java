package com.cybr47.instar.mods.devops;
...
public class DevOptionsUnlockHook {
    ...
    public void handleDevOptions(DexKitBridge bridge) {
        ...
        try {
            ...
            ModuleLog.line("(Instar | DevOptionsEnable): ❌ Error handling Dev Options: " + e.getMessage());
        }
    }
    ...
    private void findAndHookDynamicMethod(DexKitBridge bridge) {
        try {
            ...
            ModuleLog.line("(Instar | DevOptionsEnable): 🔍 Discovery Tier 1 (String)...");
            ...
            if (!found) {
                ModuleLog.line("(Instar | DevOptionsEnable): ⚠️ Tier 1 failed. Discovery Tier 2 (Structural)...");
                ...
                if (structural != null) {
                    try {
                        ...
                        ModuleLog.line("(Instar | DevOptionsEnable): 🎯 Found via structural match: "
                                + structural.getClassName() + "." + structural.getName());
                        found = true;
                    } catch (Throwable e) {
                        ModuleLog.line("(Instar | DevOptionsEnable): ❌ Structural match failed to bind: " + e.getMessage());
                    }
                }
            }
            ...
            if (!found) {
                ModuleLog.line("(Instar | DevOptionsEnable): ⚠️ Tier 2 failed. Discovery Tier 3 (Config ID)...");
                for (long configId : IS_EMPLOYEE_CONFIG_IDS) {
                    ...
                    if (!idMethods.isEmpty()) {
                        ...
                        ModuleLog.line("(Instar | DevOptionsEnable): 🎯 Found via Config ID " + configId + " in: " + targetClass);
            ...
            if (!found) {
                ModuleLog.line("(Instar | DevOptionsEnable): ❌ Tier 3 failed. Debugging global references...");
                ...
                for (MethodData m : debugMethods) {
                    ModuleLog.line("(Instar | DevOptionsDebug): String 'is_employee' found in: " + m.getClassName() + "." + m.getName());
                }
            }

        } catch (Exception e) {
            ModuleLog.line("(Instar | DevOptionsEnable): ❌ Error during discovery: " + e.getMessage());
        }
    }
    ...
    private MethodData resolveEmployeeGateStructurally(DexKitBridge bridge) {
        try {
            ...
            ModuleLog.line("(Instar | DevOptionsEnable): ❌ Structural discovery error: " + e.getMessage());
            return null;
        }
    }
    ...
    private void hookExactMethod(Method m) {
        try {
            ...
            ModuleLog.line("(Instar | DevOptionsEnable): ✅ Hooked: " + m.getDeclaringClass().getName() + "." + m.getName());
        } catch (Throwable e) {
            ModuleLog.line("(Instar | DevOptionsEnable): ❌ Failed to hook resolved method: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            for (MethodData invokedMethod : invokedMethods) {
                ...
                if (paramTypes.size() == 1 && paramTypes.get(0).contains("com.instagram.common.session.UserSession")) {
                    ...
                    ModuleLog.line("(Instar | DevOptionsEnable): 📦 Hooking via String detection: " + targetClass);
                    DexKitCache.saveString("DevOptionsClass", targetClass);
                    hookAllBooleanMethodsInClass(bridge, targetClass);
                    return true;
                }
            ...
            ModuleLog.line("(Instar | DevOptionsEnable): ❌ Error inspecting invoked methods: " + e.getMessage());
        }
        return false;
    }
    ...
    private void hookBooleanMethodsViaReflection(String className) {
        try {
            ...
            for (Method m : clazz.getDeclaredMethods()) {
                ...
                ModuleLog.line("(Instar | DevOptionsEnable): ✅ Hooked (cache): " + className + "." + m.getName());
            }
        } catch (Throwable e) {
            ModuleLog.line("(Instar | DevOptionsEnable): ❌ Reflection fallback failed: " + e.getMessage());
        }
    }
            ...
            for (MethodData method : methods) {
                ...
                if (returnType.contains("boolean") && paramTypes.size() == 1 && paramTypes.get(0).contains("com.instagram.common.session.UserSession")) {
                    try {
                        ...
                        ModuleLog.line("(Instar | DevOptionsEnable): ✅ Hooked: " + method.getClassName() + "." + method.getName());
                    } catch (Throwable e) {
                        ModuleLog.line("(Instar | DevOptionsEnable): ❌ Failed to hook " + method.getName() + ": " + e.getMessage());
                    }
                }
            }
        } catch (Exception e) {
            ModuleLog.line("(Instar | DevOptionsEnable): ❌ Error while hooking class: " + className + " → " + e.getMessage());
        }
    }
}