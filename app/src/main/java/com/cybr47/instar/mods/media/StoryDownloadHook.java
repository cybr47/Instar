...
public class StoryDownloadHook {
    ...
    private void installButtonInjectorHook(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        if (method == null) {
            try {
                ...
                if (methods.isEmpty()) {
                    ModuleLog.line("(Instar | Story) ❌ Button builder method not found");
                    return;
                }
                ...
                ModuleLog.line("(Instar | Story) ❌ Button builder DexKit: " + t);
                return;
            }
        }

        if (method == null) {
            ModuleLog.line("(Instar | Story) ❌ No CharSequence[] return type candidate found");
            return;
        }
        ...
        try {
            ...
            ModuleLog.line("(Instar | Story) ❌ Button builder hook: " + t);
        }
    }
    ...
    private void installClickHandlerHook(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        if (method == null) {
            try {
                ...
                if (methods.isEmpty()) {
                    ModuleLog.line("(Instar | Story) ❌ Click handler not found");
                    return;
                }
                method = methods.get(0).getMethodInstance(classLoader);
                DexKitCache.saveMethod("StoryDownload_click", method);
            } catch (Throwable t) {
                ModuleLog.line("(Instar | Story) ❌ Click handler DexKit: " + t);
                return;
            }
        }
        ...
        try {
            ...
            ModuleLog.line("(Instar | Story) ❌ Click handler hook: " + t);
        }
    }
    ...
    private static String extractStoryUrl(Object holder) {
        ...
        try {
            ...
            ModuleLog.line("(Instar | Story) reelItem=" +
            ...
            collectImageCandidates(target, candidates,
                    Collections.newSetFromMap(new IdentityHashMap<>()), 0);
            ModuleLog.line("(Instar | Story) imageCandidates=" + candidates.size());
            if (!candidates.isEmpty()) {
                candidates.sort((a, b) -> Integer.compare(b.area, a.area));
                ModuleLog.line("(Instar | Story) bestCandidate area=" + candidates.get(0).area
            ...
            ModuleLog.line("(Instar | Story) extractStoryUrl error: " + t);
        }
        return null;
    }
    ...
    private static String extractUsernameFromReelItemHolder(Object holder) {
        if (holder == null) {
            ModuleLog.line("(Instar | Story|Username) holder is null");
            return null;
        }
        ModuleLog.line("(Instar | Story|Username) searching in holder=" + holder.getClass().getName());
        try {
                ...
                for (Field f : cls.getDeclaredFields()) {
                    ...
                    try {
                        ...
                        if (val != null && val.getClass().getName()
                            ...
                            reelItem = val;
                            ModuleLog.line("(Instar | Story|Username) found ReelItem in field="
            ...
            if (reelItem == null && holder.getClass().getName()
                    .equals("com.instagram.model.reels.ReelItem")) {
                reelItem = holder;
                ModuleLog.line("(Instar | Story|Username) holder is itself a ReelItem");
            }
            if (reelItem == null) {
                ModuleLog.line("(Instar | Story|Username) ❌ ReelItem not found in holder");
                return null;
            }
            ...
            for (Method m : reelItem.getClass().getDeclaredMethods()) {
                ...
                try {
                    ...
                    if (candidateClass.equals("com.instagram.user.model.User")) {
                        ...
                        if (username != null) {
                            ModuleLog.line("(Instar | Story|Username) reelItem." + m.getName() + "() [User] → " + username);
                            return username;
                        }
                        continue;
                    }
                    ...
                    if (candidateClass.equals("com.instagram.feed.media.Media")) {
                        ...
                        if (username != null) {
                            ModuleLog.line("(Instar | Story|Username) reelItem." + m.getName()
                                    + "() [Media] → " + username);
                            return username;
                        }
            ...
            ModuleLog.line("(Instar | Story|Username) ❌ username not found on ReelItem methods");
        } catch (Throwable t) {
            ModuleLog.line("(Instar | Story|Username) ❌ Exception: " + t);
        }
        return null;
    }
    ...
    private void startDownload(Context ctx, String url, boolean isVideo) {
        ...
        ModuleLog.line("(Instar | Story|DL) username=" + currentStoryUsername + " mediaId=" + currentStoryMediaId
                + " file=" + fn);
