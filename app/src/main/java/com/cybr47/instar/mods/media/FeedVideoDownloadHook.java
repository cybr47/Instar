...
import com.cybr47.instar.utils.users.UserUtils;
...
public class FeedVideoDownloadHook {
    ...
    private void installUriCaptureHook() {
        try {
            ...
            ModuleLog.line("(Instar | MediaDownload): ❌ Uri.parse hook: " + t);
        }
    }
                            ...
                            if (isFeedLike) {
                                ...
                                view.setOnLongClickListener(lv -> {
                                    ...
                                    if (media != null) {
                                        ...
                                        if (url != null) {
                                            ModuleLog.line("(Instar | Reel) media tag hit, url=" + url);
                                            onDownloadClicked(ctx, List.of(url), lv);
                                            return true;
                                        }
                                        ModuleLog.line("(Instar | Reel) media tag set but no video URL found in object");
                                    }
                                    ...
                                    ModuleLog.line("(Instar | Reel) buffer fallback, m86=" + m86.size() + " total=" + all.size());
                                    // Take only the most recent URL (first in deque = newest)
                                    onDownloadClicked(ctx, List.of(pick.get(0)), lv);
                                    return true;
                                });
                                ModuleLog.line("(Instar | Reel) long-press hook set on like_button");
                            }
                        }
                    });
        } catch (Throwable t) {
            ModuleLog.line("(Instar | MediaDownload): ❌ View hook: " + t);
        }
    }
    ...
    private void injectDownloadButton(View saveBtn, ViewGroup parent,
        ...
        parent.post(() -> {
            try {
                ...
                ModuleLog.line("(Instar | DL) Cannot inject download button: " + e.getMessage());
            }
        });
    }
    ...
    private List<String> resolveUrls(View likeBtn, View downloadBtn) {
        ...
        ModuleLog.line("(Instar | DL) Tier-1a urls=" + urls.size());
        ...
        if (saveResId != 0) {
            ...
            for (int i = 0; i < 4 && p instanceof ViewGroup vg; i++, p = vg.getParent()) {
                ...
                if (realSaveBtn != null) {
                    ModuleLog.line("(Instar | DL) Tier-1b found save btn at parent level " + i);
                    urls = urlsFromSaveBtnListener(realSaveBtn);
                    ModuleLog.line("(Instar | DL) Tier-1b urls=" + urls.size());
                    if (!urls.isEmpty()) return urls;
        ...
        try {
            ...
            if (mediaClass != null) {
                ...
                if (media != null) {
                    ...
                    ModuleLog.line("(Instar | DL) stepA1 videoUrl=" + (videoUrl != null
                    ...
                    if (videoUrl == null && mutableMediaDictIntfClass != null && !carouselCandidates.isEmpty()) {
                        ...
                        ModuleLog.line("(Instar | DL) stepA2 videoUrl=" + (videoUrl != null
                                ? videoUrl.substring(0, Math.min(80, videoUrl.length())) : "null"));
                    }
                    ...
                    if (mutableMediaDictIntfClass != null && !carouselCandidates.isEmpty()) {
                        Object dictIntf = findFieldAssignableTo(media, mutableMediaDictIntfClass);
                        ModuleLog.line("(Instar | DL) dictIntf=" + (dictIntf != null
                        ...
                        if (dictIntf != null) {
                            for (Method candidate : carouselCandidates) {
                                try {
                                    ...
                                    ModuleLog.line("(Instar | Car) candidate=" + candidate.getName()
                                            + " items=" + items.size());
                                    ...
                                    for (int idx = 0; idx < items.size(); idx++) {
                                        ...
                                        if (methodImageUrl != null) {
                                            try {
                                                ...
                                                if (r instanceof String s && isCdnMediaUrl(s)) {
                                                    ModuleLog.line("(Instar | Car) item[" + idx + "] mediaExtKt=" + s.substring(0, Math.min(60, s.length())));
                                                    carouselUrls.add(s);
                                                    continue;
                                                }
                                            } catch (Throwable ignored) {}
                                        ...
                                        ModuleLog.line("(Instar | Car) item[" + idx + "] probed=" + probed);
                                    ...
                                    ModuleLog.line("(Instar | Car) carouselUrls=" + carouselUrls.size());
    ...
    public static void installVideoUrlCaptureHook(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        if (DexKitCache.isCacheValid()) {
            ...
            if (cached != null && !cached.isEmpty()) {
                ...
                ModuleLog.line("(Instar | DL|DexKit) VideoUrlCapture: " + cached.size() + " method(s) from cache");
        ...
        try {
            ...
            ModuleLog.line("(Instar | DL|DexKit) VideoVersionIntf implementors found: " + classes.size());
                ...
                try {
                    ...
                    for (MethodData methodData : methods) {
                        try {
                            ...
                            ModuleLog.line("(Instar | DL|DexKit) ✅ Hooked getUrl() on "
                                    + classData.getName());
                            hooked.add(m);
                        } catch (Throwable e) {
                            ModuleLog.line("(Instar | DL|DexKit) ❌ Hook failed for "
                                    + classData.getName() + ": " + e.getMessage());
                        }
                    }
                } catch (Throwable e) {
                    ModuleLog.line("(Instar | DL|DexKit) ❌ findMethod failed for "
                            + classData.getName() + ": " + e.getMessage());
                }
            }
            if (!hooked.isEmpty()) DexKitCache.saveMethods("VideoUrlCapture", hooked);
        } catch (Throwable e) {
            ModuleLog.line("(Instar | DL|DexKit) ❌ installVideoUrlCaptureHook: " + e.getMessage());
        }
    ...
    private static void resolveUsernameGetter(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        try {
            ...
            if (userMethods.isEmpty()) {
                ModuleLog.line("(Instar | DL|Username) ❌ username_missing_during_update not found");
                return;
            }
            ...
            ModuleLog.line("(Instar | DL|Username) userClass=" + userClass.getName());
            ...
            try {
                ...
                if (!ugMethods.isEmpty()) {
                    ...
                    ModuleLog.line("(Instar | DL|Username) userUsernameGetter=" + UserUtils.userUsernameGetter.getName());
                } else {
                    ModuleLog.line("(Instar | DL|Username) ❌ userUsernameGetter not found via -265713450");
                }
            } catch (Throwable t) {
                ModuleLog.line("(Instar | DL|Username) ❌ userUsernameGetter resolution: " + t);
            }

            resolveDictUserGetter(bridge, classLoader);

        } catch (Throwable t) {
            ModuleLog.line("(Instar | DL|Username) ❌ resolveUsernameGetter: " + t);
        }
    }
    ...
    private static void resolveDictUserGetter(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (Method m : curr.getDeclaredMethods()) {
                ...
                if (m.getParameterCount() == 0 && m.getReturnType().equals(userClass)) {
                    ...
                    ModuleLog.line("(Instar | DL|Username) ✅ Resolved dictUserGetter: " + m.getName());
                    return;
                }
            }
        ...
        try {
            ...
            if (!results.isEmpty()) {
                ...
                dictUserGetter = m;
                DexKitCache.saveMethod("DictUserGetter", m);
                ModuleLog.line("(Instar | DL|Username) ✅ Resolved dictUserGetter (concrete class): " + m.getName());
                return;
            }
        } catch (Throwable t) {
            ModuleLog.line("(Instar | DL|Username) ❌ dictUserGetter DexKit lookup: " + t);
        }

        ModuleLog.line("(Instar | DL|Username) ❌ Failed to resolve dictUserGetter in hierarchy");
    }
    ...
    static OutputStream openOutputStream(Context ctx, String filename, boolean isVideo, String username)
        ...
        if (!FeatureFlags.downloaderCustomPath.isEmpty()) {
            try {
                ...
                ModuleLog.line("(Instar|DL) Raw path failed, trying SAF: " + e.getMessage());
            }
        }
        ...
        if (!FeatureFlags.downloaderCustomUri.isEmpty()) {
            try {
                ...
                ModuleLog.line("(Instar|DL) SAF failed, falling back to MediaStore: " + e.getMessage());
            }
        }
        ...
        File dir = new File(Environment.getExternalStorageDirectory(), "Instar");
        ...
        try {
            de.robv.android.xposed.XSharedPreferences cp =
                    new de.robv.android.xposed.XSharedPreferences(
                            "com.cybr47.instar", "instar_cache");
            cp.reload();
    ...
    private static void delegateUrlToCompanionApp(Context ctx,
        ...
        intent.setClassName("com.cybr47.instar",
                            "com.cybr47.instar.mods.media.DownloadSaveService");
        ...
        ModuleLog.line("(Instar | DL) Delegated to DownloadSaveService: " + filename);
    }
    ...
    static List<String> extractAllUrlsFromMedia(Context ctx, Object media) {
        ...
        ModuleLog.line("(Instar | Post|DEBUG) carousel check: mutableMediaDictIntfClass=" +
        ...
        if (mutableMediaDictIntfClass != null && !carouselCandidates.isEmpty()) {
            Object dictIntf = findFieldAssignableTo(media, mutableMediaDictIntfClass);
            ModuleLog.line("(Instar | Post|DEBUG) dictIntf=" +
            ...
            if (dictIntf != null) {
                for (Method candidate : carouselCandidates) {
                    try {
                        ...
                        ModuleLog.line("(Instar | Post|DEBUG)   candidate=" + candidate.getName() +
        ...
        if (urls.size() == 1) {
            ...
            executor.submit(() -> {
                try {
                    ...
                    ModuleLog.line("(Instar | Post|DL) single failed: " + e);
                    mainHandler.post(() -> Toast.makeText(ctx,
    ...
    private static void showCarouselBottomSheet(Context ctx, List<String> urls,
        ...
        try {
                ...
                executor.submit(() -> {
                    ...
                    for (String url : urls) {
                        ...
                        try {
                            ...
                            failed++;
                            ModuleLog.line("(Instar | Post|DL) item failed: " + e);
                        }
                    }
            ...
            ModuleLog.line("(Instar | Post) ❌ showCarouselBottomSheet: " + t);
        }
    }
    ...
    private void onDownloadClicked(Context ctx, List<String> urls, View saveBtn) {
        ...
        ModuleLog.line("(Instar | DL) onDownloadClicked username=" + currentDownloadUsername + " mediaId=" + currentDownloadMediaId);
        ...
        ModuleLog.line("(Instar | DL) total=" + urls.size()
                + " videos=" + videos.size() + " images=" + images.size());
        for (int i = 0; i < videos.size(); i++)
            ModuleLog.line("(Instar | DL) video[" + i + "]=" + videos.get(i));
        for (int i = 0; i < images.size(); i++)
            ModuleLog.line("(Instar | DL) image[" + i + "]=" + images.get(i));
    ...
    private void handleMixedContent(Context ctx, List<String> allUrls,
        ...
        executor.submit(() -> {
            ...
            ModuleLog.line("(Instar | DL) probeUrl=" + videoUrl
    ...
    private void startDirectDownload(Context ctx, String url, boolean isVideo) {
        ...
        ModuleLog.line("(Instar | DL) startDirectDownload file=" + fn);
        ...
        executor.submit(() -> {
            try {
                ...
                ModuleLog.line("(Instar | DL) download failed: " + e.getClass().getSimpleName() + ": " + e.getMessage());
                mainHandler.post(() -> Toast.makeText(ctx,
                        I18n.t(ctx, R.string.ig_toast_download_failed, e.getMessage()), Toast.LENGTH_SHORT).show());
            }
        });
        ...
        executor.submit(() -> {
            ...
            if (!uri.isEmpty()) {
                ...
                try {
                    ...
                    ModuleLog.line("(Instar | DL) merge delegate failed: " + e.getMessage());
